...
AgenDAV publishes no container image to any registry. `docker-compose.yml`
therefore builds [upstream's own production
Dockerfile](https://github.com/agendav/agendav/blob/main/Dockerfile) from a
pinned git tag, which costs a few minutes of composer and npm work on first
run.
...
- **Upgrading** means bumping the tag in `docker-compose.yml` and re-running
  `./init.sh`, which rebuilds and re-applies migrations.
